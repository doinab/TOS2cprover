#include "controllers/msp430_hw.h"

pthread_t main_thr;                  /* PATCH */
pthread_mutex_t gie;                 /* PATCH */
...
inline static void __nesc_enable_interrupt( void ) 
{
  pthread_mutex_unlock(&gie);   /* PATCH */
}
...
__nesc_atomic_t __nesc_atomic_start( void ) 
{
  ...
  __nesc_atomic_t result = __x & 0x0008 != 0;
  __nesc_disable_interrupt();
  return result;
}
...
void sig_TIMERB0_VECTOR( void ) 
{
  __nesc_disable_interrupt(); /* PATCH */
  Msp430TimerCommonP_VectorTimerB0_fired();
  __nesc_enable_interrupt(); /* PATCH */
}
...
static uint16_t Msp430TimerP_1_Timer_get( void ) 
{
/*  PATCH: remove all this but the return of _TBR */
//  if(1)
//  {
//    uint16_t t0;
//    uint16_t t1 = _TBR;
...
int main( void ) 
{
  set_initial_configuration();         /* PATCH: initial state */
  ...
  /* PATCH */
  pthread_mutex_init(&gie, NULL);
  pthread_create(&main_thr, NULL, main_thread, NULL);
  pthread_create(&intB0_thr, NULL, sig_TIMERB0_VECTOR, NULL);
