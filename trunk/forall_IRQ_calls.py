...
while loop:
  ...
  if res > -1: 
    ...
    irq_type = contents[res+len(irqstr):res+len(irqstr)+7]
    print '\n' + '-'*80
    ...
    if irq_type == 'PORT1_V':
      os.system('./c outrf_' + str(loopidx) + '.c sig_PORT1_VECTOR.1')
    if irq_type == 'PORT2_V':
      os.system('./c outrf_' + str(loopidx) + '.c sig_PORT2_VECTOR.1')
    if irq_type == 'UART0TX':
      os.system('./c outrf_' + str(loopidx) + '.c sig_UART0TX_VECTOR.1')
    if irq_type == 'UART0RX':
      os.system('./c outrf_' + str(loopidx) + '.c sig_UART0RX_VECTOR.1')
    sys.stdout.flush()
