...
class Struct(Node):
    def __init__(self, name, decls, packed, coord=None):
        self.name = name
        self.decls = decls
        self.packed = packed
        self.coord = coord
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        ...
        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("name", repr(self.name)), ("packed", repr(self.packed))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.name, self.packed])
        buf.write(attrstr)
...
class FuncDecl(Node):
    def __init__(self, args, type, quals, coord=None):
        self.args = args
        self.type = type
        self.quals = quals
        self.coord = coord
